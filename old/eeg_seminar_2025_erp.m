...
xlabel('Error Attribution');
ylabel('Mean Value');
legend({'reward low', 'reward high'});
title('P3 mean amplitude');
xlim([0.5, 3.5])
grid on;


%% Plot N2 component at Fz

idx_channel = 15;
erp_timewin = [1800, 1850];
erp_average_fz = squeeze(erp_average(:, idx_channel, :));
figure()

subplot(2, 2, 1)
plot(EEG.times, mean(erp_average_fz, 1), 'k', 'LineWidth', 2)
xline([0, 1500])
ylims = ylim;
x_patch = [erp_timewin(1), erp_timewin(2), erp_timewin(2), erp_timewin(1)];
y_patch = [ylims(1), ylims(1), ylims(2), ylims(2)];
patch(x_patch, y_patch, [1 0.5 0], 'FaceAlpha', 0.3, 'EdgeColor', 'none');
title(['Grand Average N2'])
xlim([-200, 2500])
...
ylabel('Mean Value');
legend({'reward low', 'reward high'});
title('N2 mean amplitude');
xlim([0.5, 3.5])
grid on;



%% Plot CNV component at FCz


idx_channel = 64;
erp_timewin = [1200, 1500];
erp_average_fcz = squeeze(erp_average(:, idx_channel, :));
figure()

subplot(2, 2, 1)
plot(EEG.times, mean(erp_average_fcz, 1), 'k', 'LineWidth', 2)
xline([0, 1500])
ylims = ylim;
x_patch = [erp_timewin(1), erp_timewin(2), erp_timewin(2), erp_timewin(1)];
y_patch = [ylims(1), ylims(1), ylims(2), ylims(2)];
patch(x_patch, y_patch, [1 0.5 0], 'FaceAlpha', 0.3, 'EdgeColor', 'none');
title(['Grand Average CNV'])
xlim([-200, 2500])
