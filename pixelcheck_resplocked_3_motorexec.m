...
for s = 1 : n_subjects
    ...
    % Load
    load([PATH_TF_DATA, id_string{1}, '_itpcs.mat']); % cond x chan x freq x time

    error_diffs(s, 1, :, :, :) = squeeze(itpcs(13, :, :, :));
    error_diffs(s, 2, :, :, :) = squeeze(itpcs(14, :, :, :));
    error_diffs(s, 3, :, :, :) = squeeze(itpcs(1, :, :, :));
    error_diffs(s, 4, :, :, :) = squeeze(itpcs(2, :, :, :));
    error_diffs(s, 5, :, :, :) = squeeze(itpcs(7, :, :, :));
    error_diffs(s, 6, :, :, :) = squeeze(itpcs(8, :, :, :));

end % end subject loop

% Set label for analysis
analysis_label = 'resplocked_motor';

% Posterior electrode patch
idx_channel = [5, 6]; % frontal midline

idx_time = tf_times >= -500 & tf_times <= -300;
idx_freq = tf_freqs >= 8 & tf_freqs <= 30;
